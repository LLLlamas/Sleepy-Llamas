...
|---|---|---|
| 1789351236 | 09-13 | **Deleting a record no longer crashes the app.** It always had: the delete lands on `CareStore`'s own context, so the screen still held the deleted record and reading it was an uncatchable `fatalError` — a correction at 3am took the app down. Also: the **Share** control on Summary had no name, so VoiceOver read out the symbol and nothing could find it. Check: open any record from the timeline, Delete, confirm — the entry goes and Undo is offered; then Share and check both documents are listed. |

...
LEAKED=$(strings "$BINARY" | grep -cE \
  "moonlogSeedDemo|moonlogOpenSheet|moonlogEditFirst|moonlogShiftHours|moonlogDemoWrite|moonlogDumpHandoff|moonlogSettingsSheet|moonlogResetStore|moonlogStillGlyphs" \
  || true)
if [ "$LEAKED" -ne 0 ]; then
